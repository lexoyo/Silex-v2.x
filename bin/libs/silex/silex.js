...
org.slplayer.component.list.List.prototype = $extend(org.slplayer.component.ui.DisplayObject.prototype,{
		...
		while(_g1 < _g) {
			...
			if(idxElem >= 0) {
				...
				if(children[idx] == null) {
					haxe.Log.trace("--workaround--" + idx + "- " + Std.string(children[idx]),{ fileName : "List.hx", lineNumber : 245, className : "org.slplayer.component.list.List", methodName : "updateSelectionDisplay"});
					continue;
				}
	...
	,getItemID: function(childElement) {
		...
		var idx = element.getAttribute("data-list-item-idx");
		while(idx == null && element != this.rootElement && element != null) {
			element = element.parentNode;
			idx = element.getAttribute("data-list-item-idx");
		}
		if(idx == null) throw "Error, could not find the element clicked in the list.";
	...
	,attachListEvents: function() {
		...
		while(_g1 < _g) {
			var idx = _g1++;
			children[idx].setAttribute("data-list-item-idx",Std.string(idx));
		}
	}
	,reloadData: function() {
		this.doRedraw();
	...
	,doRedraw: function() {
		...
		while(_g1 < _g) {
			var i = _g1++;
			haxe.Log.trace("wanna clean " + Std.string(this.rootElement.childNodes[i]),{ fileName : "List.hx", lineNumber : 129, className : "org.slplayer.component.list.List", methodName : "doRedraw"});
		...
		while(_g1 < _g) {
			var i = _g1++;
			haxe.Log.trace("wanna init " + Std.string(this.rootElement.childNodes[i]),{ fileName : "List.hx", lineNumber : 136, className : "org.slplayer.component.list.List", methodName : "doRedraw"});
...
org.slplayer.util.DomTools.inspectTrace = function(obj,callingClass) {
	haxe.Log.trace("-- " + callingClass + " inspecting element --",{ fileName : "DomTools.hx", lineNumber : 157, className : "org.slplayer.util.DomTools", methodName : "inspectTrace"});
	var _g = 0, _g1 = Reflect.fields(obj);
	while(_g < _g1.length) {
		var prop = _g1[_g];
		++_g;
		haxe.Log.trace("- " + prop + " = " + Std.string(Reflect.field(obj,prop)),{ fileName : "DomTools.hx", lineNumber : 160, className : "org.slplayer.util.DomTools", methodName : "inspectTrace"});
	}
	haxe.Log.trace("-- --",{ fileName : "DomTools.hx", lineNumber : 162, className : "org.slplayer.util.DomTools", methodName : "inspectTrace"});
}
...
org.slplayer.util.DomTools.setBaseTag = function(href) {
	...
	if(baseNodes.length > 0) {
		haxe.Log.trace("Warning: base tag already set in the head section. Current value (\"" + baseNodes[0].getAttribute("href") + "\") will be replaced by \"" + href + "\"",{ fileName : "DomTools.hx", lineNumber : 341, className : "org.slplayer.util.DomTools", methodName : "setBaseTag"});
...
silex.component.ComponentModel.prototype = $extend(silex.ModelBase.prototype,{
	...
	,addComponent: function(nodeName,layer) {
		var publicationModel = silex.publication.PublicationModel.getInstance();
...
silex.layer.LayerModel.prototype = $extend(silex.ModelBase.prototype,{
	...
	,addLayer: function(page,layerName) {
		var publicationModel = silex.publication.PublicationModel.getInstance();
...
silex.property.PropertyModel.prototype = $extend(silex.ModelBase.prototype,{
	...
	,setStyle: function(viewHtmlDom,name,value) {
		haxe.Log.trace("setStyle(" + Std.string(viewHtmlDom) + ", " + name + ", " + value + ")",{ fileName : "PropertyModel.hx", lineNumber : 197, className : "silex.property.PropertyModel", methodName : "setStyle"});
		var modelHtmlDom = silex.publication.PublicationModel.getInstance().getModelFromView(viewHtmlDom);
		try {
	...
	,setProperty: function(viewHtmlDom,name,value) {
		haxe.Log.trace("setProperty(" + Std.string(viewHtmlDom) + ", " + name + ", " + Std.string(Type["typeof"](value)) + ")",{ fileName : "PropertyModel.hx", lineNumber : 153, className : "silex.property.PropertyModel", methodName : "setProperty"});
		var modelHtmlDom = silex.publication.PublicationModel.getInstance().getModelFromView(viewHtmlDom);
		try {
	...
	,setAttribute: function(viewHtmlDom,name,value) {
		haxe.Log.trace("setAttribute(" + Std.string(viewHtmlDom) + ", " + name + ", " + Std.string(Type["typeof"](value)) + ")",{ fileName : "PropertyModel.hx", lineNumber : 102, className : "silex.property.PropertyModel", methodName : "setAttribute"});
		var modelHtmlDom = silex.publication.PublicationModel.getInstance().getModelFromView(viewHtmlDom);
		try {
			if(value == null) {
...
silex.publication.PublicationModel.prototype = $extend(silex.ModelBase.prototype,{
	...
	,onData: function(publicationData) {
		...
		var headOpenIdx = this.currentData.html.indexOf("<head");
		if(headOpenIdx == -1) headOpenIdx = this.currentData.html.indexOf("<HEAD");
		var headCloseIdx = this.currentData.html.indexOf("</head>");
		if(headCloseIdx == -1) headCloseIdx = this.currentData.html.indexOf("</HEAD>");
		var bodyOpenIdx = this.currentData.html.indexOf("<body");
		if(bodyOpenIdx == -1) bodyOpenIdx = this.currentData.html.indexOf("<BODY");
		var bodyCloseIdx = this.currentData.html.indexOf("</body>");
		if(bodyCloseIdx == -1) bodyCloseIdx = this.currentData.html.indexOf("</BODY>");
		if(headOpenIdx > -1 && headCloseIdx > -1) {
			var closingTagIdx = this.currentData.html.indexOf(">",headOpenIdx);
...
silex.page.PageModel.NEW_LAYER_NAME = "body";
